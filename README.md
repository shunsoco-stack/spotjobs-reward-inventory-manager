...
2026年9月21日の最新ローカル結果：lint・型検査・ビルド成功、単体テスト170件成功。54本すべて早期補充の4,050円、および19/20/49/50/99/100/149/150本の境界を含みます。ブラウザテストは17件成功・1件スキップで、Xの投稿URLと今日／1週間の切替をDesktop Chromium、Pixel 7相当、iPhone 13相当の3構成で確認しました。スキップは既知のWebKitオフライン再起動テストです。スクショ取り込みは実画像でも読み取り・訂正・保存・取消・重複除外を別途確認しています。検証に使った個人画像・認識文字は公開リポジトリへ含めていません。X共有を含む最新状態の本番確認は、次回のProduction公開後に行います。
