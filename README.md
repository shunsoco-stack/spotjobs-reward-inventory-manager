...
「Xで共有画面を開く」を押すと、`#SPOTJOBS記録 #SPOTJOBS`を含む集計文を入れた`x.com`の投稿画面を別タブで開きます。**投稿は自動で確定せず**、X側で内容を確認して投稿します。店舗名、メモ、個別の履歴、現在庫は共有文へ含めません。プレビューの表示までは端末内で完結し、Xの投稿画面を開く操作にはネットワーク接続が必要です。
