...
補充と抜取を手早く記録して、「今週いくら？」「あと何本？」を確認する、SPOTJOBSワーカー向けの個人制作・非公式Webアプリです。
...
スマートフォンでの入力しやすさを中心に、補充・抜取・報酬・在庫をひとつの画面構成にまとめました。本数を入力すると、基本報酬、早期補充、週間本数ボーナス、手元の在庫を計算します。
...
| 機能 | できること |
| --- | --- |
| ホーム | 週間報酬の内訳、補充・抜取本数、現在庫、目標、次のボーナスを確認 |
| 補充入力 | 大きな数字入力、増減ボタン、+1・+2・+3・+5・+10、地域、早期対象、メモ |
| 抜取入力 | 日付と本数を記録し、在庫と早期判定に反映 |
| 履歴 | 日付・地域・作業種類で絞り込み、編集・削除、直前の変更を取り消し |
| 週次報酬 | 基本・早期・本数ボーナスの内訳、日別実績、日別・地域別の補充集計 |
| 在庫・棚卸 | 今日・昨日・一昨日の在庫、抜取日ごとの残り、棚卸差異を確認 |
...
初期設定は、**抜取日を含む3日以内・対象1本につき10円**です。抜取記録がある在庫を古い順に消費したと仮定し、早期対象の本数を推定します。

補充入力では「自動判定」「すべて対象」「対象外」「本数指定」を選べます。実際の条件と推定が異なる場合は手動で指定してください。抜取日が不明な在庫や、抜取未記録の本数には早期ボーナスを自動加算しません。
...
```text
現在庫 ＝ 前日繰越 ＋ 抜取 − 補充 ＋ 反映済みの棚卸調整
```

今日・昨日・一昨日の表示は、日付が変わると繰り越します。抜取日ごとの残りと、今日までの早期対象も確認できます。

抜取の記録が足りない場合は、理論在庫をマイナスのまま表示し、記録漏れを知らせます。不足分を自動的に0へ丸めたり、存在しない抜取記録を作ったりしません。

## 棚卸

実際に数えた本数を記録し、記録上の在庫との差分を確認できます。初期状態は**確認記録のみ**で、棚卸しただけでは在庫を変更しません。

「棚卸数を在庫に反映する」を選ぶと、変更前後の本数と差異を確認してから反映します。確認のみ・反映済みの状態は履歴に残ります。棚卸で増えた在庫の抜取日は不明として扱い、自動の早期対象から外します。
...
2026年9月18日にVercel本番環境で撮影した実際の画面です。ホーム・補充入力・在庫の3枚はスマートフォン幅、週次報酬・シミュレーターの2枚はデスクトップ幅です。個人の作業履歴は使わず、アプリのデモモードで撮影しています。[撮影条件と画像の記録](docs/screenshots/capture.json)も保存しています。
...
主なテスト対象は、報酬の段階境界、週の区切り、単価保存、先入先出、負在庫、棚卸、シミュレーション、JSON移行・復元、IndexedDBの保存完了・競合、CSVの引用符・列対応・不正行です。IndexedDBの単体テストはfake-indexeddb上で行い、利用者のデータは使いません。

テストファイルは `src/lib/core.test.ts`、`src/lib/storage.test.ts`、`src/lib/csv.test.ts`。GitHub Actionsでもlint、型検査、テスト、ビルド、静的ファイルの生成を確認する構成です。

2026年9月18日の結果：lint・型検査・ビルド成功、単体テスト99件成功。54本すべて早期補充の4,050円、および19/20/49/50/99/100/149/150本の境界を含みます。本番URLのブラウザテストは14件成功・1件スキップ、コンソールエラー0件でした。
...
プレビューは [http://localhost:3059](http://localhost:3059)。`next build` と `scripts/build-offline.mjs` により `out/` へ静的ファイルとService Workerを生成します。Vercelでは同梱の `vercel.json` を使い、`out/` 全体を配信します。ほかの静的ホスティングではHTTPSとルート配信を用意してください。
