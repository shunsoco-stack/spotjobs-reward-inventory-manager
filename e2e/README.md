...
2026年9月21日に、最新のビルド済み静的プレビューとVercel Productionで次の共有フローを検証しました。
...
同じシナリオをDesktop Chromium、Pixel 7 Chromium、iPhone 13 WebKitで実行し、ローカルとProductionの両方で3構成すべて成功しました。全体では各環境17件成功、Windows WebKitの通信断中再起動だけを理由付きで1件スキップしています。単体テストは170件です。実行したProductionテストのコンソールエラーは0件でした。投稿画面を開くにはネットワーク接続と、必要に応じてXへのログインが必要です。
