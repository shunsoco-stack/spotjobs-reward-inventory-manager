...
const titles: Record<View, string> = { home: 'ホーム', refill: '補充入力', pickup: '抜取入力', history: '履歴', settings: '設定', inventory: '在庫・棚卸', weekly: '週次報酬', simulator: '報酬シミュレーター' };
const nav = [{ id: 'home', label: 'ホーム', icon: House }, { id: 'refill', label: '補充', icon: ArrowDownToLine }, { id: 'pickup', label: '抜取', icon: ArrowUpFromLine }, { id: 'history', label: '履歴', icon: History }, { id: 'settings', label: '設定', icon: Settings2 }] as const;
  ...
  return <div className="app-shell">
    ...
    <div className="main-shell"><header className="app-header"><div className="mobile-brand"><Brand/></div><span className="desktop-header">SPOTJOBSワーカーのための管理ツール</span><span className={`connection-status ${!online ? 'offline' : ''}`}>{online ? <ShieldCheck size={16}/> : <WifiOff size={16}/>}<span>{!online ? 'オフライン' : saving ? '保存中' : '端末に保存'}</span></span></header>
      <main id="main-content">
        ...
        {view === 'history' && <HistoryView summary={summary} settings={data.settings} today={today} onEdit={setEditing}/>}
