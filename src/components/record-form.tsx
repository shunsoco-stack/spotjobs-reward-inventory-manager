...
export default function RecordForm({ data, today, type, record, onSave }: Props) {
  ...
  return <form className={`record-form ${type}`} onSubmit={submit}>
    <div className="form-intro"><span className={`round-icon ${type}`}><Icon size={24}/></span><div><h2>{record ? `${typeLabel(type)}の記録を編集` : type === 'adjustment' ? '手元の本数を確認' : `${typeLabel(type)}を記録`}</h2><p>{type === 'refill' ? '本数を入れたら、そのまま保存。' : type === 'pickup' ? '抜き取った本数を在庫に追加します。' : '実際に数えた本数を入力してください。'}</p></div></div>
